...
def fill_years(df, years, value):
    ...
    for i, year in enumerate(years):
        ...
        if "2010" in year:
            ...
            is_in_year = df["status_q"].dt.strftime("%Y") == year_str
...
def init_developments():
    ...
    developments_co["effectivedate"] = pd.to_datetime(developments_co["effectivedate"])
    ...
    developments_co = developments_co[
        developments_co["effectivedate"] <= pd.Timestamp.now()
    ]
...
def new_building_unit_correction(developments):
    ...
    for i in range(len(years) - 1):
        ...
        current_year_greater = new_building_developments[current_year].astype(
            "float"
        ) > new_building_developments[next_year].astype("float")
        new_building_developments.loc[
            current_year_greater, current_year
        ] = new_building_developments.loc[current_year_greater, next_year]
...
def main():
    ...
    developments = developments.merge(df_job_groups, how="left", on="job_number")
    ############# Fill in values ###############
    ...
    developments[years] = developments[years].fillna(value=pd.np.nan).astype(float)

    is_non_alteration_developments = ~(
        (developments["job_type"] == "Alteration") & (developments["units_net"] == "0")
    )

    unit_change = developments[years] - developments[years].shift(axis="columns")
    years.remove("units_init")
    developments.loc[is_non_alteration_developments, years] = unit_change.loc[
        is_non_alteration_developments, years
    ]
    ...
    developments = developments.rename(
        columns={
            "unit_change_2010_a_PreCensus": "unit_change_preapr2010",
            "unit_change_2010_b_PostCensus": "unit_change_postapr2010",
        }
    )

    developments = developments.drop(
        columns=["unit_change_2007", "unit_change_2008", "unit_change_2009"]
    )
    developments = developments.rename(
    ...
    developments = developments.loc[
        ~(developments.iloc[:, unit_change_loc:] == 0).all(axis=1)
    ]
    developments = developments.loc[
        ~(developments.iloc[:, unit_change_loc:].isna()).all(axis=1)
    ]
