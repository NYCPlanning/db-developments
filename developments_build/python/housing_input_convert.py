...
def rename_field(c):
    if c == 'dcp_occ_category': return 'occ_category'
    if c == 'dcp_occ_pr': return 'occ_prop'
    if c == 'prop_stories': return 'stories_prop'
    if c == 'u_net_comp': return 'units_complete'
    if c == 'u_net_inc': return 'units_incomplete'
    else: return c
...
if __name__ == '__main__':
    ...
    old_dev = pd.read_sql("SELECT job_number, bbl, bin, units_net,\
    ...
    yearly_unitschange = pd.read_sql("SELECT *, unit_exist_preapr2010 AS unit_change_preapr2010\
    ...
    df = df[df.reason != 'HNY manual match'].drop(columns=['ogc_fid','v','hny_id','extra'])
    df = df.melt(['job_number','reason'], var_name='field', value_name='new_value').sort_values(by=['job_number','field'])
    ...
    old_dev = old_dev.melt('job_number', var_name='field', value_name='old_value').sort_values(by=['job_number','old_value'])
    old_dev = old_dev[~old_dev.old_value.isna()]

    print('melting old yearly_unitschange without correction...')
    unit_change = list(filter(lambda x: 'unit_change_' in x, list(yearly_unitschange.columns)))[1:]
    yearly_unitschange = yearly_unitschange[['job_number'] + unit_change]
    yearly_unitschange = yearly_unitschange.melt('job_number', var_name='field', value_name='old_value').sort_values(by=['job_number','old_value'])
