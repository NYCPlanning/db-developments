...
if __name__ == '__main__':
    ...
    corr = pd.read_sql("SELECT job_number, field, old_value, new_value\
    ...
    stable_columns = ['job_number', 'job_type', 'status_q', 'status', 'units_init', 'units_prop', 'units_net', 'x_mixeduse']
    df = df.melt(stable_columns, var_name='field', value_name='value').sort_values(by = stable_columns+['field'])

    df['job_number'] = df.job_number.astype('str')
    df = pd.merge(df, corr, how = 'left', on = ['job_number', 'field'])

    df['value'] = np.where((df.value==df.old_value)|(df.old_value == None), df.new_value, df.value)
    ...
    df = df[stable_columns + unit_change].rename(columns={'unit_change_preapr2010':'unit_exist_preapr2010'})
    
    print('dump into postgres...')
    df.to_sql('yearly_unitchange', engine, if_exists='replace', chunksize=2000, index=False)