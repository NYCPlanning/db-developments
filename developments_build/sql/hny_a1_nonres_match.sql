/** Match by bin&bbl, limited to A1s or non-residentials 
and projects not matched by residential NBs**/
WITH match AS(
    SELECT 
        h.hny_id,
        d.job_number,
    ...
    WHERE (d.job_type = 'Alteration'
    OR d.occ_category != 'Residential'
...
/** Match by bbl only, limited to A1s or non-residentials and projects not matched by BIN&BBL**/
WITH match AS(
    SELECT 
        h.hny_id,
        d.job_number,
...
/** Match spatially, limited to A1s or non-residentials and projects not matched by bin&bbl, or bbl only**/
WITH match AS(
    SELECT 
        h.hny_id,
        d.job_number,
